...
const themeToggleButtonMobile = document.getElementById("theme-toggle-m");
...
function setDarkTheme() {
	...
	themeToggleButtonMobile.innerHTML = SUN_SVG;
...
function setLightTheme() {
	...
	themeToggleButtonMobile.innerHTML = MOON_SVG;
...
document.addEventListener('scroll', () => {
	let scrolledPercentage = (scrollContainer().scrollTop / (scrollContainer().scrollHeight - scrollContainer().clientHeight)) * 100;
	progressBar.style.width = `${scrolledPercentage}%`;
	scrollContainer().scrollTop > 100 ? backToTop.classList.remove('hidden') : backToTop.classList.add('hidden');
}),
